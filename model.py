...
# Override LayerNormalization with BatchNormalization
...
def attention_block(x, dim=16):
    ...
    gap_gmp = tf.keras.layers.Conv2D(
        dim, (3, 3), strides=(1, 1), padding="same", activation="sigmoid"
    )(gap_gmp)
    ...
    x1 = tf.keras.layers.Conv2D(
        dim, (1, 1), strides=(1, 1), padding="same", activation="relu"
    )(gap)
    x1 = tf.keras.layers.Conv2D(
        dim, (1, 1), strides=(1, 1), padding="same", activation="sigmoid"
    )(x1)
    ...
    x2 = tf.keras.layers.Conv2D(
        dim, (1, 1), strides=(1, 1), padding="same", activation=None
    )(attention)
...
def inv_block(x, channels=3):
    ...
    x = Conv2D(
        channels, (1, 1), activation="relu", strides=(1, 1), padding="same"
    )(x)
    y = Add()([m, x])
    return y
...
def __get_model(inputs_med, inputs_low, encoder_dim, out_dim, n_encoders=3):
    ...
    for d in range(len(encoder_dim)):
        ...
        x = tf.keras.layers.Conv2DTranspose(
            feats, (3, 3), strides=(2, 2), padding="same"
        )(x)
...
def naive_multires_pyramid(image, weight_map, levels):
    ...
    for i in range(levels, 0, -1):
        ...
        imgLpyr.append(
            imgGpyr[i - 1]
            - tf.image.resize(pyrUp(imgGpyr[i]), shape[-3:-1])
        )
...
def get_model(shape, batch_size=None, resize_output=False):
    ...
    uv_low_input = tf.keras.layers.Input(
        (HEIGHT // 4, WIDTH // 4, 2), batch_size=batch_size
    )
    y_med_input = tf.keras.layers.Input((HEIGHT, WIDTH, 1), batch_size=batch_size)
    uv_med_input = tf.keras.layers.Input(
        (HEIGHT // 4, WIDTH // 4, 2), batch_size=batch_size
    )
    ...
    Ew = (
        tf.abs(u_inputs + tf.keras.backend.epsilon())
        * tf.abs(v_inputs + tf.keras.backend.epsilon())
        + 1
    )
    ...
    Cw = tf.keras.layers.DepthwiseConv2D(
        ...
        strides=(1, 1),
        padding="same",
        depthwise_initializer=lambda shape, dtype: kernel,
    )(y_inputs)
    Cw = tf.abs(Cw) + 1
    ...
    Gn = tf.keras.layers.DepthwiseConv2D(
        (5, 5),
        strides=(1, 1),
        padding="same",
        depthwise_initializer=tf.keras.initializers.Constant(value=1 / 25.0),
    )(weight_maps)
    ...
    model = tf.keras.models.Model(
        inputs=[y_low_input, uv_low_input, y_med_input, uv_med_input],
        outputs=[outY, outUV],
    )
    return model
...
def convert_to_onnx():
    ...
    for size in [512, 768, 1024, 1280, 1536, 1792, 2048, 4096]:
        ...
        input_signature = [
            tf.TensorSpec(inp.shape, tf.float32, name=inp.name) for inp in model.inputs
        ]
...
def convert_prediction(y_pred, uv_pred):
    ...
    rgb_pred = np.uint8(tf.clip_by_value(rgb_pred * 255.0, 0.0, 255.0))
...
def get_arguments():
    ...
    parser.add_argument(
        "--height",
        type=int,
        help="Input height of the images.",
        default=2816,
    )
    parser.add_argument(
        "--width",
        type=int,
        help="Input width of the images.",
        default=4096,
    )
